...
// ── Phases ────────────────────────────────────────────────────────────────────
export async function listPhases(goalId) {
...
// ── Milestones ────────────────────────────────────────────────────────────────
export async function listMilestones(goalId, phaseId) {
    const sb = getSupabase();
...
export async function completeMilestone(id, completed) {
...
// ── Tasks ─────────────────────────────────────────────────────────────────────
export async function listTasks(date, goalId) {
    const sb = getSupabase();
...
// ── Ideas ─────────────────────────────────────────────────────────────────────
export async function listIdeas(verdict) {
    const sb = getSupabase();
...
export async function getDashboard() {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const tasksArr = todayTasks ?? [];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const goalsArr = goals ?? [];
    // Per-goal phase summary
    const goalSummaries = await Promise.all(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    goalsArr.map(async (g) => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const phases = (await listPhases(g.id)) ?? [];
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const milestones = (await listMilestones(g.id)) ?? [];
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const activePhase = phases.find((p) => p.phase_number === g.current_phase);
        return {
            id: g.id,
            title: g.title,
            progress: g.progress,
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            milestones_done: milestones.filter((m) => m.completed).length,
        };
    }));
    return {
        date: today(),
        ...
        today_tasks: {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            completed: tasksArr.filter((t) => t.completed).length,
            tasks: tasksArr,
        },
...
export async function seedFromCareerAgent(userName, userSkills, userContext) {
    // 1. Set up profile
    await upsertProfile({
        name: userName,
        skills: userSkills,
