...
async function callTool(name, args) {
    switch (name) {
        ...
        case "setup_new_user": return handle(() => db.upsertProfile({
            name: args.name,
            skills: args.skills ?? [],
        ...
        case "import_career_agent": return handle(() => db.seedFromCareerAgent(args.name, args.skills ?? [], args.context ?? ""));
        case "reset_all_data": return handle(async () => {
            if (!args.confirmed)
                return { skipped: true, reason: "confirmed must be true" };
            return db.resetAllData();
        });
        // Profile
        case "get_profile": return handle(db.getProfile);
        case "update_profile": return handle(() => db.upsertProfile({
            name: args.name,
            skills: args.skills,
            current_focus: args.current_focus,
            context: args.context,
        ...
        // Goals
        case "list_goals": return handle(() => db.listGoals(args.status));
        ...
        case "update_goal": return handle(async () => {
            const updated = await db.updateGoal(args.goal_id, {
                title: args.title,
                description: args.description,
                category: args.category,
                status: args.status,
        ...
        // Phases
        case "list_phases": return handle(() => db.listPhases(args.goal_id));
        ...
        case "update_phase_status": return handle(() => db.updatePhaseStatus(args.phase_id, args.status));
        // Milestones
        case "list_milestones": return handle(() => db.listMilestones(args.goal_id, args.phase_id));
        ...
        case "complete_milestone": return handle(() => db.completeMilestone(args.milestone_id, args.completed ?? true));
        // Tasks
        case "list_tasks": return handle(() => db.listTasks(args.date, args.goal_id));
        ...
        case "complete_task": return handle(() => db.completeTask(args.task_id, args.completed ?? true));
        case "delete_task": return handle(() => db.deleteTask(args.task_id));
        // Ideas
        case "list_ideas": return handle(() => db.listIdeas(args.verdict));
        case "add_idea": return handle(() => db.addIdea({
            title: args.title,
            description: args.description,
        }));
        case "update_idea": return handle(() => db.updateIdea(args.idea_id, {
            verdict: args.verdict,
            score: args.score,
            reasoning: args.reasoning,
            integration_note: args.integration_note,
            risk_note: args.risk_note,
            resources: args.resources,
            related_goal_ids: args.related_goal_ids,
        }));
        case "delete_idea": return handle(() => db.deleteIdea(args.idea_id));
        // Reflections
        case "log_reflection": return handle(async () => {
            const result = await db.logReflection({
                ...
                wins: args.wins,
                gaps: args.gaps,
                tomorrow_priority: args.tomorrow_priority,
                notes: args.notes,
                reflection_date: args.reflection_date,
                goal_progress_deltas: args.goal_progress_deltas,
            });
            // Apply progress deltas to goals
            if (args.goal_progress_deltas) {
                const deltas = args.goal_progress_deltas;
                for (const [goalId, delta] of Object.entries(deltas)) {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const goal = (await db.getGoal(Number(goalId)));
                    if (goal) {
        ...
        case "get_reflections": return handle(() => db.getReflections(args.limit));
        default:
...
export function createServer() {
    const server = new Server({ name: "nudge-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
